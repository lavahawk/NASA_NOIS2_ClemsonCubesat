...
const gridLabelStyle: LayerProps = {
  ...
  layout: {
    ...
    // Interpolation: Text gets larger as you zoom in
    'text-size': [
      'interpolate',
      ['linear'],
      ['zoom'],
    ...
    // Prevent labels from crashing into each other
...
function App() {
  ...
  const [zoom, setZoom] = useState(4); // Track zoom for grid density

  // Popup State
  const [selectedFire, setSelectedFire] = useState<{ lng: number; lat: number; props: any; } | null>(null);

  // Dynamic Grid Calculation based on Zoom
  const gridStep = useMemo(() => {
    if (zoom <= 3) return 20; // Zoomed out: sparse grid
    if (zoom <= 5) return 10;
    if (zoom <= 7) return 5;
    if (zoom <= 9) return 2;
    return 1;                 // Zoomed in: dense grid
  }, [zoom]);
  ...
  const onMapLoad = useCallback((e: any) => {
    ...
    setZoom(e.target.getZoom()); // Set initial zoom
  }, []);

  const onMove = useCallback((e: any) => {
    setZoom(e.viewState.zoom); // Update zoom state when panning/scrolling
  }, []);
    ...
    <div style={{ display: 'flex', height: '100vh', width: '100vw', fontFamily: 'sans-serif', overflow: 'hidden' }}>
      ...
      <div className="map-container" style={{ flex: 1, position: 'relative' }}>
        ...
        >
          ...
          {selectedFire && (
            ...
            >
              <div style={{ color: '#333' }}>
                <h3 style={{ margin: '0 0 8px 0', borderBottom: '1px solid #ccc', paddingBottom: '4px' }}>
                  {selectedFire.props.name || `Fire ID: ${selectedFire.props.id}`}
                </h3>
        ...
        <div style={{ position: 'absolute', top: '20px', right: '20px', zIndex: 10, display: 'flex', gap: '10px' }}>
          <button
            ...
            style={{
              padding: '12px 16px', backgroundColor: showGrid ? '#ff8c00' : 'white', color: showGrid ? 'white' : '#333',
              border: '1px solid #ccc', borderRadius: '8px', cursor: 'pointer', fontWeight: 'bold', boxShadow: '0 4px 6px rgba(0,0,0,0.1)'
            }}
          >
          ...
          <button
            ...
            style={{
              padding: '12px 24px', backgroundColor: 'white', color: '#333', border: '1px solid #ccc',
          ...
          {!isDashboardOpen && (
            <button
              ...
              style={{
                padding: '12px 24px', backgroundColor: '#333', color: 'white', border: 'none',
                borderRadius: '8px', cursor: 'pointer', fontWeight: 'bold', boxShadow: '0 4px 6px rgba(0,0,0,0.3)'
              }}
            >
              Enter Dashboard ➔
            </button>
      ...
      {isDashboardOpen && (
        <aside style={{ width: '300px', padding: '20px', backgroundColor: '#f8f9fa', borderLeft: '1px solid #ccc', display: 'flex', flexDirection: 'column' }}>
          ...
          <div style={{ marginTop: '30px', padding: '15px', backgroundColor: 'white', borderRadius: '6px', boxShadow: '0 1px 3px rgba(0,0,0,0.1)' }}>
